...
%%%%%%%%% ABSTRACT
\begin{abstract}
In this paper we are presenting our results from using Conditional Random Fields(CRF) to segment words in the Zulu language. A CRF model with different generic feature functions was used to segment novel Zulu words. The feature functions captured patterns in prefixes and suffixes, vowel/consonant patterns and patterns for windows of three to seven consecutive specific letters. The weights were learned using Collin’s Perceptron Algorithm and Stochastic Gradient Descent (SGD). Our model obtained about 74\% word accuracy and 96\% letter level accuracy.
\end{abstract}
%%%%%%%%% BODY TEXT
%INDICES:
% words n=1...N
...
\begin{align}
        \hat{y} & =\argmax_{\y} \, p(\yb\,|\,\xb;\w) \\
                &= \argmax_{\y} \sum_{j=1}^J w_j F_j(\xb,\yb).
                \label{equation}
\end{align}
...
\begin{align}
        ...
        %&= \sum_{n=1}^N \left( F_j(\x_n,y_n) -  \sum_{\y'} p(\y'\,|\,\x_n ;\,\w)\,F_j(\xb_n, \y' ) \right)  \notag \\
...
\begin{align}
        \mathbb{E}_{\y'\,|\,\xb_n} \left[ F_j(\x_n, \y' ) \right] &= \sum_{\y'} p(\y'\,|\,\x_n ;\,\w)\,F_j(\x_n, \y' ). \label{Eq:expectation}
\end{align}
...
\section{Results}
To test the algorithms the accuracy was measured using a 4-fold cross validation. Using Collin’s Perceptron, the average word accuracy was about 74\%. The letter level accuracy was about 96\%. The results can be seen in Table \ref{resultsCollins}.

The results using SGD are shown in Table \ref{resultsSGD}. The overall results are slightly worse than using Collin’s; 65\% of words are correct and 94\% of tags. One explanation of this is that the perceptron algorithm is trained explicitly to maximize accuracy, while SGD is trying to maximize the $LCL$.
...
\begin{table}
	\begin{tabular}{|l|l|l|}
    	...
    	\hline
	\end{tabular}
	\caption{Prediction Accurarcy using SGD with the Viterbi algorithm. Test 3 was omitted.}
\label{resultsSGD}
\end{table}
...
\centering
\begin{minipage}{7 cm}
	\includegraphics[width=7cm]{figures/weights.eps}
\end{minipage}
\caption{Feature weights (Test 2). Collors correspond to the type of feature: (blue--purple) 3--7 windows, (yellow) prefix and suffix, (black) consonant-vowel features}
\label{Fig:weights}
\end{figure}
